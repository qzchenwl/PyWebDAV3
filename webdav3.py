    ...
    def addChild(self, tag):
        ...
        if tag.name in self.d:
            if type(self.d[tag.name]) == type(
                    self):  # If there are multiple sibiling tags with same name, form a list :)
...
class DAVRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_MOVE(self):
        ...
        newfile = self.server.root.rootdir() + urllib.parse.urlparse(
            urllib.parse.unquote(self.headers['Destination'])).path
    ...
    def do_PROPFIND(self):
        ...
        def write_props_member(w, m):
            w.write('<D:response>\n<D:href>%s</D:href>\n<D:propstat>\n<D:prop>\n' % urllib.parse.quote(
                m.virname))  # add urllib.quote for chinese
